...
# Default cwd when a fresh topic has no project bound and none is configured.
# A persistent, private (0700) scratch dir beats /tmp: working files survive
# reboot/tmp-cleanup, aren't world-readable, and pathless Glob/Grep stay
# scoped to an empty dir (nothing to leak) — the "not $HOME" reason for the
# old /tmp default. Session transcripts live under ~/.claude regardless, so
# resume is unaffected either way.
PLAYGROUND_DIR = TGCLAUDE_DIR / "playground"
PLAYGROUND_DIR.mkdir(mode=0o700, exist_ok=True)
...
class LiveStatus:
    ...
    that actually sends resolves it — quoting the turn's originating message
    — and clears it, so every later segment resolves None and sends
    unquoted. A single turn can't tell which segment answers which of
    several steered messages, so it quotes only the origin and stops
    guessing; the next turn re-establishes a fresh quote target."""
    ...
    def _resolve(self):
        if self.anchor is self._UNSET:
            self.anchor = self.anchor_fn() if self.anchor_fn else None
        return self.anchor

    async def _send(self, update_obj: Update, text: str, **kw):
        anchor = self._resolve()
        if anchor is not None:
            return await anchor.reply_text(text, **kw)
        return await update_obj.effective_message.reply_text(
...
async def run_turn(
    ...
    """Fire ONE input into the CLI and return. The reply is emitted by the
    conv's pump (the single continuous consumer of the stream), so there is
    no per-turn receive here and no ResultMessage is ever left buffered —
    input and output are decoupled, like the native stream. `status_text`/
    `anchor` are kept for call-site compatibility but unused (the pump shows
    its own working status; anchor precision is a later step)."""
    ...
    try:
        await ctx.bot.send_chat_action(
            chat_id=conv.key[0], message_thread_id=conv.key[1] or None,
    ...
    try:
        ...
        conv.pending.clear()
        if sender_id is None:
            _inflight_del(conv, msg)
            try:
                await msg.set_reaction(None)
            except BaseException:
                pass
        try:
            if is_owner(update):
...
async def _fwd_flush(key: tuple) -> None:
    ...
    # every batch member waits for the pump to clear it on the turn's
    # ResultMessage (its inflight was persisted in _fwd_add). run_turn marks
    # the representative (update.effective_message); add the rest to pending
    # so the pump releases them too — NOT here (run_turn only fires the query;
    # the reply hasn't landed yet, so clearing now would be premature).
    for m, _, _ in items:
        if m is not update.effective_message and m not in conv.pending:
            conv.pending.append(m)
...
async def _recover_conv(app: Application, key: ConvKey, ent: dict) -> None:
    ...
    try:
        ...
        pass
    try:
        # ensure_client starts the pump; the recovery reply streams back
        # through it, exactly like any other turn (no private receive loop
        # that would fight the pump for the stream)
        client = await ensure_client(conv)
